...
export function validForm(
  ...
  let valid_tag_difficulty = "";

  const tags_dropdown = document.getElementById("tags-dropdown");
  const tags_importance = document.getElementById("tags-importance");

  const default_select_easy = document.getElementById("default-select-easy");
  const default_select_medium = document.getElementById(
    "default-select-medium"
  );
  const default_select_hard = document.getElementById("default-select-hard");

  const selected_tag = document.getElementById("selected-tag");
  const selected_tag_easy = document.getElementById("selected-tag-easy");
  const selected_tag_medium = document.getElementById("selected-tag-medium");
  const selected_tag_hard = document.getElementById("selected-tag-hard");
  ...
  const tags_dropdown_open = document.getElementById('tags-dropdown-open');
  const tags_dropdown_close = document.getElementById('tags-dropdown-close');

  tags_dropdown.addEventListener("click", () => {
    tags_importance.classList.toggle("hidden");

    if (tags_dropdown_close.classList.contains('hidden')) {
        tags_dropdown_open.classList.add('hidden');
        tags_dropdown_close.classList.remove('hidden');
    } else {
        tags_dropdown_open.classList.remove('hidden');
        tags_dropdown_close.classList.add('hidden');
    }
  });

  default_select_easy.addEventListener("click", () => {
    valid_tag_difficulty = "easy";

    selected_tag.classList.remove("hidden");
    tags_importance.classList.add("hidden");

    selected_tag_easy.classList.remove("hidden");
    selected_tag_medium.classList.add("hidden");
    selected_tag_hard.classList.add("hidden");

    tags_dropdown_open.classList.add('hidden');
    tags_dropdown_close.classList.remove('hidden');
  });

  default_select_medium.addEventListener("click", () => {
    valid_tag_difficulty = "medium";

    selected_tag.classList.remove("hidden");
    tags_importance.classList.add("hidden");

    selected_tag_easy.classList.add("hidden");
    selected_tag_medium.classList.remove("hidden");
    selected_tag_hard.classList.add("hidden");
  ...
  default_select_hard.addEventListener("click", () => {
    valid_tag_difficulty = "hard";
    ...
    selected_tag_easy.classList.add("hidden");
    selected_tag_medium.classList.add("hidden");
    selected_tag_hard.classList.remove("hidden");
  ...
  selected_tag.addEventListener("click", () => {
    selected_tag.classList.add("hidden");
  });
  ...
  form.addEventListener("submit", (e) => {
    e.preventDefault();
    let task_name_input = document
      .getElementById("task-name-input")
      .value.trim();
    ...
    if (
      valid_tag_difficulty !== "" &&
      task_name_input !== "" &&
      description_input !== ""
    ) {
      console.log("yesss");
      if (valid_tag_difficulty === "easy") {
        selected_tag_easy.classList.add("hidden");
        generated_tasks_easy.push({
          task_not_completed: `<div
            ...
            >
              <div class="flex gap-4 w-[90%]">
                ...
                <div class="flex flex-col gap-4">
                  ...
                  <p class="text-[12px] text-[#7D7D7F] dark:text-[#848890] font-light break-all whitespace-pre-line leading-tight">${description_input}
                  </p>
                </div>
              </div>
        ...
        localStorage.setItem('easy-tasks', JSON.stringify(generated_tasks_easy));

        console.log(generated_tasks_easy)
      } else if (valid_tag_difficulty === "medium") {
        selected_tag_medium.classList.add("hidden");
        generated_tasks_medium.push({
          task_not_completed: `<div
              class="flex justify-between px-4 py-3 rounded-xl border border-[#E9E9E9] dark:border-none dark:bg-[#091120] relative"
              id="need-to-done-task"
              data-counter="${counter}"
        ...
        localStorage.setItem('medium-tasks', JSON.stringify(generated_tasks_medium));

        console.log(generated_tasks_medium);
      } else {
        selected_tag_hard.classList.add("hidden");
        generated_tasks_hard.push({
          task_not_completed: `<div
              class="flex justify-between px-4 py-3 rounded-xl border border-[#E9E9E9] dark:border-none dark:bg-[#091120] relative"
              id="need-to-done-task"
              data-counter="${counter}"
        ...
        localStorage.setItem('hard-tasks', JSON.stringify(generated_tasks_hard));

        console.log(generated_tasks_hard);
      }

      document.getElementById("task-name-input").value = "";
      document.getElementById("description-input").value = "";
      today_tasks_need_to_done.innerHTML = '';
      ...
      const number = generated_tasks_easy.length + generated_tasks_medium.length + generated_tasks_hard.length;
      number_of_uncompleted_tasks.textContent = `${number} تسک را باید انجام دهید.`;
      localStorage.setItem('number_of_uncompleted_tasks', `${number} تسک را باید انجام دهید.`);
    }
  });
}
